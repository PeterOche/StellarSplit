...
describe('Dispute Resolution System - Integration Tests', () => {
    ...
    it('POST /disputes/:disputeId/resolve - should resolve dispute', async () => {
      const response = await request(app.getHttpServer())
        ...
        .send({
          ...
          resolution: 'Dispute verified. Participant will receive credit of $25.',
  ...
  describe('Appeal Mechanism', () => {
    it('POST /disputes/:disputeId/appeal - should appeal resolved dispute', async () => {
      ...
      const response = await request(app.getHttpServer())
        ...
        .send({
          appealReason: 'The resolution was biased and did not consider all evidence',
        })
        .expect(200);
  ...
  describe('State Machine Validation', () => {
    ...
    it('should reject invalid state transitions', async () => {
      ...
      expect(resolveResponse.body.message).toContain('Invalid dispute status transition');
    });
  });
  ...
  describe('Admin Operations', () => {
    ...
    it('should request more evidence', async () => {
      ...
      const response = await request(app.getHttpServer())
        ...
        .send({
          evidenceRequest: 'Please provide photos of the items received and the packing slip',
        })
        .expect(200);
  ...
  describe('Query and Filtering', () => {
    ...
    it('GET /disputes?status=resolved - should filter by status', async () => {
      ...
      expect(response.body.disputes.every((d: any) => d.status === DisputeStatus.RESOLVED)).toBe(true);
    });
  ...
  describe('Notification Events', () => {
    it('should emit events when dispute is created', async () => {
      ...
      expect(eventSpy).toHaveBeenCalledWith('dispute.created', expect.any(Object));
